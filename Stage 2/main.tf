...
resource "azurerm_network_interface_backend_address_pool_association" "WebApp" {
...
resource "azurerm_network_interface_backend_address_pool_association" "WebApp2" {
  network_interface_id    = azurerm_network_interface.nic["WebApp2"].id
  ip_configuration_name   = azurerm_network_interface.nic["WebApp2"].ip_configuration[0].name
  backend_address_pool_id = azurerm_lb_backend_address_pool.lb.id
...
#Added session persistence
  load_distribution              = split(",", each.value)[3]
  loadbalancer_id                = azurerm_lb.lb.id
  probe_id                       = azurerm_lb_probe.lb[each.key].id
  resource_group_name            = azurerm_resource_group.rg.name
  frontend_ip_configuration_name = var.lb["Frontend"].name
  backend_address_pool_ids       = [azurerm_network_interface_backend_address_pool_association.WebApp.backend_address_pool_id]
}
# ---------------------------------------------------------------------------------------------------------------------
#   Network Interface
# ---------------------------------------------------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------------------------------------
#   Virtual Machine: WebApp / MySQL / WebApp2
# ---------------------------------------------------------------------------------------------------------------------
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  #Stage2: Added Availability Zone
